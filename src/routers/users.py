...
async def login(login_form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    print(login_form_data.password, user.hashed_password, user.salt)
    
    verified = Security.verify_password(login_form_data.password, user.hashed_password, user.salt)
    if verified:
...
async def signup(new_user: UserBase, password: str = Body()):
    ...
    await UsersCollection.create(new_user, salt, hashed_password)
    return new_user
...
async def me(user: TokenData = Depends(Security.current_user)):
    return UsersCollection.get(user.id)
